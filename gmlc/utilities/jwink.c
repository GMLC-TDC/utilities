...
float jaro_winkler_comp(char *s1, char *s2)
{
	...
	char str_large[26]; /*26 instead of 25 to accomodate a termination Character*/
	char str_small[26];
	int j_max=0;
	int j_min=0;
	...
	//printf("c=%d, t=%d, l1=%d, l2=%d, f=%d, half_length=%d\n",c,t,len_s1, len_s2,f,half_length);
	Result=((float)((float)c/(float)len_s1)+(float)((float)c/(float)len_s2)+((float)t/(float)c))/3.0f;
