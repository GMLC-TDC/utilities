...
class TimeSeriesMulti {
  ...
  private:
    std::vector<timeType> m_time;  //!< a vector of times associated with the
                                   //!< m_data
    std::vector<std::vector<dataType>> m_data;  //!< a 2d vector of m_data to
...
*/
    ...
    void loadBinaryFile(const std::string& fileName)
    {
        ...
        setCols(rcount - 1);  // update the number of columns the file contains
