...
float dp_comp(char *n1, char *n2)
{
	...
	for (ii=1;ii<strlen(n2);ii++)
	{
		for (jj=1;jj<strlen(n1);jj++)
		{
			DV[ii][jj]=MAX(DV[ii-1][jj-1]+charcomp[(int)n1[jj]][(int)n2[ii]],MAX(DV[ii][jj-1],DV[ii-1][jj]));
			if ((ii>1)&&(jj>1))
			{
				if ((DV[ii-1][jj]==DV[ii][jj-1])&&(DV[ii-1][jj]>DV[ii-1][jj-1])&&(DV[ii-1][jj-1]==DV[ii-2][jj-2]))
				{
