...
{
    X sum_of_vector =
      std::accumulate(a.begin(), a.end(), X(0),
                      [](X a1, X a2) -> X { return a1 + std::abs(a2); });
    return sum_of_vector;
}
...
{
    X prod_of_vector = std::accumulate(a.begin(), a.end(), X(0),
                                       [](X a1, X a2) { return a1 * a2; });
                ...
                std::vector<X> &M)
{
    auto cnt = (std::min)(a.size(), b.size());
    M.resize(cnt);
    std::transform(a.begin(), a + cnt, b.begin(), M.begin(),
                   std::multiplies<X>());
}
                                   ...
                                   std::false_type /*unused*/)
{
    ...
    std::transform(dvec.begin(), dvec.end(), ret.begin(),
                   [](Y val) { return static_cast<X>(val); });
    return ret;
}
/** perform a vector conversion of one type to another
@details uses SFINAE to do some checking to determine if the types are actually
...
{
    return vectorConvertDetail::vectorConvertActual<X, Y>(dvec,
                                                          std::is_same<X, Z>{});
}
