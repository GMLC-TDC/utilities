...
float SW_comp(char *n1, char *n2)
{
	...
	*/
	...
	for (ii=1;ii<=l2;ii++)
	{
		for (jj=1;jj<=l1;jj++)
		{
			DV[ii][jj]=MAX(DV[ii-1][jj-1]+charcomp[(int)c1[jj-1]][(int)c2[ii-1]],MAX(0,MAX(DV[ii][jj-1]-gap,DV[ii-1][jj]-gap)));
			if ((ii>1)&&(jj>1))
			{
				if ((DV[ii][jj-1]>DV[ii-1][jj-1])&&(DV[ii-1][jj]>DV[ii-1][jj-1]))
				{
	...
	if ((DV[l2][l1]-DV[l2-1][l1]>0.001)&&(DV[l2][l1]-DV[l2][l1-1]>0.001))
	{
		maxj=l1;
		maxi=l2;
	}
	else
	{
		...
		for (jj=l1-1;jj>2;jj--)
		{
			if ((DV[l2][jj]>=1.95)&&(DV[l2][jj]-DV[l2][jj-1]>0.001)&&(DV[l2][jj]-DV[l2][jj+1]>0.001))
			{
				maxj=jj;
			}
		}
		...
		for (ii=l2-1;ii>2;ii--)
		{
			if ((DV[ii][l1]>=1.95)&&(DV[ii][l1]-DV[ii-1][l1]>0.001)&&(DV[ii][l1]-DV[ii+1][l1]>0.001))
			{
				maxi=ii;
			}
		}
	...
	*/
	...
	for (kk=numwords2-1;kk>=0;kk--)
	{
		...
		maxj=l1;
		if (!((v2[l1]==0)&&(DV[ii][l1]-DV[ii-1][l1]>0.001)&&(DV[ii][l1]-DV[ii][l1-1]>0.001)&&(DV[ii][l1]-DV[ii+1][l1]>0.001)))
		{
			for (nn=l1-1;nn>2;nn--)
			{
				if ((v2[nn]==0)&&(DV[ii][nn]-DV[ii][nn-1]>0.001)&&(DV[ii][nn]-DV[ii][nn+1]>0.001))
				{
					maxj=nn;
				}
			}
	...
	for (kk=numwords1-1;kk>=0;kk--)
	{
		...
		maxi=l2;
		if (!((v1[l2]==0)&&(DV[l2][jj]-DV[l2][jj-1]>0.001)&&(DV[l2][jj]-DV[l2-1][jj]>0.001)&&(DV[l2][jj]-DV[l2][jj+1]>0.001)))
		{
			for (nn=l2-1;nn>2;nn--)
			{
				if ((v1[nn]==0)&&(DV[nn][jj]-DV[nn-1][jj]>0.001)&&(DV[nn][jj]-DV[nn+1][jj]>0.001))
				{
					maxi=nn;
				}
			}
